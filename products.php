...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    if(isset($_POST['id']) && isset($_SESSION['user']) && is_numeric($_POST['id']) && $_POST['id'] > 0){
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary bg-gradient">
        <div class="container">
            <a class="navbar-brand" href="./">SAL</a>
        </div>
    </nav>
                    ...
                    <div class="card rounded-0 shadow">
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                <div class="card-title col-auto flex-shrink-1 flex-grow-1">Products list</div>
                                <div class="col-atuo">
                                    <button class="btn btn-primary btn-sm btn-flat" id="add"><i class="fa fa-plus-square"></i> New product</button>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="container-fluid">
                                ...
                                <form id="member-form" action="" method="POST">
                                    ...
                                    <div class="mb-3">
                                        <label for="name" class="form-label">SKU ID</label>
                                        <input type="number" class="form-control rounded-0" id="name" name="sku" required="required" value="<?= isset($data->sku) ? $data->sku : '' ?>">
                                    </div>
                                    <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control rounded-0" id="price" name="price" required="required" value="<?= isset($data->price) ? $data->price : '' ?>">
                                    </div>
                                    <div class="mb-3">
                                        <label for="description" class="form-label">Description</label>
                                        <textarea rows="3" class="form-control rounded-0" id="description" name="description" required="required"><?= isset($data->description) ? $data->description : '' ?></textarea>
                                    </div>
                                </form>
                            </div>
                        </div>
                        <div class="card-footer text-center">
                                <button class="btn btn-primary rounded-0" form="member-form"><i class="fa-solid fa-save"></i> Save Product</button>
